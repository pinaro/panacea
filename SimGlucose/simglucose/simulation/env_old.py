...
class NS_T1DSimEnv_old(object):
    def __init__(self, patient, sensor, pump, scenario, speed, oracle):
        self.patient = patient
        self.sensor = sensor
        self.pump = pump
        self.scenario = scenario
        ...
        self.oracle = oracle
        self.frequency = speed * 0.003
        self.init_Vmx = self.patient._params.Vmx
        self.init_kp3 = self.patient._params.kp3
    ...
    def reset(self):
        ...
        # Vary it from half to double range
        # x' \in [0.5x, 2x]
        # lb, ub = 0.5, 2
        lb, ub = 0.33, 3
        # lb, ub = 0.25, 4

        # Variable to move range of cos [-1, 1] to range [lb, ub]
        multiplier = (ub - lb)/2
        offset = multiplier + lb

        if self.oracle >= 0:
            self.patient._params.Vmx = self.init_Vmx * (np.cos(self.oracle * self.frequency) * multiplier + offset)
            self.patient._params.kp3 = self.init_kp3 * (np.cos(self.oracle * self.frequency) * multiplier + offset)
        else:
            if self.speed != 0:
                self.patient._params.Vmx = self.init_Vmx * (np.cos(self.episode * self.frequency) * multiplier + offset)
                self.patient._params.kp3 = self.init_kp3 * (np.cos(self.episode * self.frequency) * multiplier + offset)
