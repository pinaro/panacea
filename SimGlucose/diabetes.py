...
def main():
    ...
    print('Min return: ', mini)
    print('Max return: ', maxi)
#    cr_c_b, cf_c_b = pick_policy_distribution()
#    print('C of CR_b: ', cr_c_b)
#    print('C of CF_b: ', cf_c_b)
#    with open(PATH+'diabates_data.csv', 'w+') as f:
#        f.write('CR\tCF\tSampledCR\tSampledCF\tReward\tNormReturn\n')
#        for i in range(0, NUM_SAMPLES):
...
#            observation, reward, done, info = env.step([cr, cf])
#            env.reset() # does this need to happen?
#            norm_r = normalize_return(reward)
#            f.write(str(cr_c_b)+'\t'+str(cf_c_b)+'\t'+str(cr)+'\t'+str(cf)+'\t'+str(reward)++'\t'+str(norm_r)+'\n')
#            f.flush()
